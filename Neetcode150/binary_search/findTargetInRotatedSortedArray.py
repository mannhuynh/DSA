...
class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        # Step 1: Find the pivot index where the array is rotated
        ...
        while left < right:
            ...
            if nums[mid] > nums[right]:  
                # This implies that the sorted order has been restored in this part of the array
                left = mid + 1  # So we move the left pointer to mid + 1 for further search
            else:
        ...
        # Step 2: Perform binary search on the non-rotated part of the array (left subarray)
        ...
        while left <= right:
            ...
            if nums[left] <= nums[mid]:
                ...
                if nums[left] <= target < nums[mid]:  
                    ...
                    right = mid - 1  # So we move the right pointer to mid - 1
                else:
                    # Otherwise, we need to keep searching in the right part of this subarray
                    left = mid + 1  # So we move the left pointer to mid + 1
            else:
